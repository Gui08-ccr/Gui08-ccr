window.addEventListener("load",function(){
    ...
    $(".container-dialog").on('click', '#accept', function() {
        $.post('http://vrp/dialogAction', JSON.stringify({ action: true }));
    });

    $(".container-dialog").on('click', '#cancel', function() {
        $('.container-dialog').fadeOut();
        $.post('http://vrp/fechar', JSON.stringify({}));
    });
	...
	window.addEventListener("message",function(evt){
		...
		if (data.dialog) {
            ...
            $('.container-dialog footer h1').html(data.npc)
            $('.container-dialog footer p').html(data.mensagem)
            $('.container-dialog #accept').html('<i class="fas fa-comments"></i>' + data.botao)
        }
