...
-- REANIMAR
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent('reanimar-npc')
AddEventHandler('reanimar-npc',function()
	...
	repeat
		...
		if IsPedDeadOrDying(ped) and not IsPedAPlayer(ped) and distance <= 1.5 and reviver == nil then
			...
			SetTimeout(15000,function()
				...
				vRP._stopAnim(false)
				TriggerServerEvent("reanimar:pagamento018301983190838131")
		...
		if prisioneiro then
			...
			if distance2 <= 100 and reducaopenal then
				...
				if distance2 <= 1.2 then
					...
					if IsControlJustPressed(0,38) then
						reducaopenal = false
						TriggerServerEvent("diminuirpena0918324209183242")
						vRP._DeletarObjeto()
					end
				end
			end
